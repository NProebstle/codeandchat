...
import { Message, CloneVisitor } from '@angular/compiler/src/i18n/i18n_ast';
...
export class ChatComponent {
  ...
  constructor() {
    ...
    send(){
      if(this.nickCheck() && this.messageCheck()){
        ...
        if(!this.longMessage){
          ...
          this.message = '';
          this.createMessage(this.output);
    ...
    createMessage(output){
      ...
      if(c == 0){
        this.createMessageElement(output);
      }
      else if(this.prevMessageID == checkID) {
        this.mergeMessages(output);
      }
      else {
        this.createMessageElement(output);
      }
    ...
    createMessageElement(output){
      ...
      div.innerHTML = `<h4>${this.nickName}:</h4><p>${output}</p><span style="position:absolute;bottom: 10px;right:10px;padding-bottom: 5px;font-size: 14px;float: right;">${timestamp}</span>`;
    ...
    messageCheck(){
      if(!/^\s*$/.test(this.message)){
        ...
        if(this.message.length > 65){
          this.messageArray = this.message.match(/(.{1,65})/g);
